...
>
  3. you should also add the following 2 roxygen comments in the general help page of the
package:
```{r, eval=FALSE}
#' @useDynLib mypkgr
...
We are now going to create a first function in `Rcpp` to invert a matrix. For this, we will use the [C++ library `Armadillo`](http://arma.sourceforge.net/docs.html). It is a modern and simple linear algebra *library*, highly optimized, and interfaced with `r fontawesome::fa("r-project")` via the `RcppArmadillo` package.
...
>
  4. Using Hadley Wickham's [introduction to `Rcpp`]](http://adv-r.had.co.nz/Rcpp.html#rcpp-intro) in his book *Advanced R*, as well as the documentation for the [`RcppArmadillo` package](http://gallery.rcpp.org/articles/armadillo-eigenvalues/) and for the `C++` library [Armadillo](http://arma.sourceforge.net/docs.html), try to write a short function `invC()` in `C++` that computes the inverse of a matrix.
> 
...
As a general rule, not much computational time is gained by replacing an optimized `r fontawesome::fa("r-project")` function with a `C++` function. Indeed, most of the `base` `r fontawesome::fa("r-project")` fuunctions are in fact already wrappers around well optimized `C` or `Fortran` routines. The gain is then limited to the suppression of argument checking and type management (which is there for a reason!).

> `r emoji::emoji("point_right")` ***Your turn !*** 
>
1. From `mvnpdfsmart`, propose a complete implementation in `C++` for computating the density of the multivariate Normal distribution `mvnpdfC()`.
> 
  2. Evaluate the performance gain of this new implementation `mvnpdfC`.


You can download our proposal for `mvnpdfC.cpp` [here](https://r-dev-perf.borishejblum.science/FormationRavancee_dev_perf_files/mvnpdfC.cpp).
